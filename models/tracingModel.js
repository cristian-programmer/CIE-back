...
class TracingModel {
  ...
  async getAllAttendance(id) {
    ...
    return await this.database.queryCommand(
      `SELECT * FROM mydb.Attendance WHERE idEvent = ${id}`
    );
  }
  ...
  async updateAttendance(id, confirmed) {
    const result = await this.database.queryCommand(
      `UPDATE mydb.Attendance SET confirmedAssistance=${
        confirmed == true ? 1 : 0
  ...
  async updateAttended(id, attend) {
    ...
    const result = await this.database.queryCommand(
      `UPDATE mydb.Attendance SET attended=${
        attend == true ? 1 : 0
      } WHERE idattendance=${id} `
    );
  ...
  async countAttendend() {
    return await this.database.queryCommand(
      `SELECT COUNT(*) FROM mydb.Attendance WHERE attended =1`
    );
  }

  async getLastActivitySystem(idUser) {
    return await this.database.queryCommand(
      `SELECT * FROM mydb.lastActivitySystem WHERE idUser=${idUser}`
    );
  }
  ...
  async createLastActivitySystem(data) {
    ...
    const result = await this.database
      .queryCommand(`INSERT INTO mydb.lastActivitySystem (idUser, activity, date, hour)
             values (${data.idUser}, "${
      data.activity
    }", "${new Date().toLocaleDateString()}", "${hour}")`);
  ...
  async getTotal() {
    return await this.database
      .queryCommand(`SELECT COUNT(*) AS total FROM mydb.Users
  ...
  async getAmountByColumn(type) {
    return await this.database
      .queryCommand(`SELECT COUNT(*) AS amount FROM mydb.Students
        WHERE incomeBy=${type}`);
  }

  async getAmountByRelationship(relationship) {
    return await this.database
      .queryCommand(`SELECT COUNT(*) AS amount FROM mydb.Users
        WHERE  relationshipUniversity='${relationship}'`);
  }

  async getAcademicPrograms() {
    return await this.database.queryCommand(
      `SELECT academicProgram FROM mydb.Students`
    );
  }
  ...
  async getProjectsByPhases() {
    return await this.database.queryCommand(
      `SELECT methodologicalPhases FROM mydb.Projects`
    );
  }
}
