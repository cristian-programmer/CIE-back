...
router.post("/updateAttendance", async (req, res) => {
  ...
  const response = await tracing.updateAttendance(
    req.body.idattendance,
    req.body.confirm
  );
  res.json({ result: response });
});
...
router.post("/updateAttended", async (req, res) => {
  ...
  const response = await tracing.updateAttended(
    req.body.idattendance,
    req.body.attended
  );
  res.json({
...
router.get("/entrepreneursServed", async (req, res) => {
  ...
  const data = new Array(
    pratice[0].amount,
    gradeOption[0].amount,
    internationalization[0].amount,
    extern[0].amount,
    graduate[0].amount,
    total[0].total
  );
...
router.get("/entrepreByProgram", async (req, res) => {
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    programs.add(students[i].academicProgram);
  }
...
router.get("/stateByPhase", async (req, res) => {
  const labels = ["Preincubación", "Incubación", "Aceleración", "Otras"];
  let pre = 0,
    inc = 0,
    ace = 0,
    other = 0;

  const response = await tracing.getProjectsByPhases();
  console.log(response);

  for (let i = 0; i < response.length; i++) {
    const phases = response[i].methodologicalPhases.split(",");

    for (let i = 0; i < phases.length; i++) {
      if (phases[i] === "Preincubación") {
        pre++;
      } else if (phases[i] === "Incubación") {
        inc++;
      } else if (phases[i] === "Aceleración") {
        ace++;
      } else {
        other++;
      }
