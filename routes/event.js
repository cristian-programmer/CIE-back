...
const FileServer = require("../infrastructure/ManagerFilesServer")
  .ManagerFileServer;
...
router.post("/updateEventStatistics", async (req, res) => {
  ...
  if (number.length > 0) {
    number =
      req.body.type == 1
        ? number[0].numberRegistered
        : number[0].numberAttendees;
    number = number + Number(req.body.count);
    response = await event.updateEventStatics(
      number,
      req.body.id,
      req.body.type
    );
...
router.post(
  "/uploadFile",
  fileServer.middleware().single("eventFile"),
  async (req, res) => {
...
router.get("/getRegisteredByEvent", async (req, res) => {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    data.push(statistics[0].numberRegistered);
  }
...
router.post(
  "/uploadPost",
  fileServer.middleware().single("photo"),
  async (req, res) => {
    console.log("uploadFile", req.file.filename);
    const uploaded = await fileServer.uploadS3File(req.file);
